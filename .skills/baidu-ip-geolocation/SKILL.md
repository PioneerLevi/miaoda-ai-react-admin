...
- **Endpoint**：`GET https://app-bv9t0unkehhd-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip`
...
async function callIpLocation(
  ...
  const url = `https://app-bv9t0unkehhd-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip${
...
serve(async (req: Request): Promise<Response> => {
  ...
  if (ip) params.set("ip", ip);
  if (coor) params.set("coor", coor);
  const upstreamUrl = `https://app-bv9t0unkehhd-api-79jK62Ze2pQL-gateway.appmiaoda.com/location/ip${
    params.toString() ? "?" + params.toString() : ""
  }`;
